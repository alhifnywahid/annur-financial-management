 ...
 *
 * `nama` is unique because it is used as the de-facto join key: `bulanan_user`
 * and every payment lookup match members by name, so two members called "Budi"
 * would silently share one payment history.
 */
 ...
 *
 * One row per member per month: the unique constraint is what makes
 * `addPayment`'s single UPDATE ... WHERE (month, nama) unambiguous, and it stops
 * a re-run of the month seeding from creating a second balance for the same
 * member.
 */
export const bulananUser = pgTable(
	"bulanan_user",
	{
		id: serial("id").primaryKey(),
		dataBulananId: integer("data_bulanan_id")
	...
	(t) => ({
		bulananNamaUnique: unique("bulanan_user_bulanan_nama_unique").on(
			t.dataBulananId,
			t.nama,
		),
	}),
