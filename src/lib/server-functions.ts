...
export const getDataBulanan = createServerFn({ method: "GET" }).handler(
	async (): Promise<DataBulananResponse> => {
		...
		const hitungDataBulan = dataBulananDTO
			.flatMap((data) => data.user.map((u) => u.total_bayar))
			.reduce((a, b) => a + b, 0);

		const totalSaldo = hitungDataBulan + uangMasuk - uangKeluar;
		const listHutang = hitungHutang(dataBulananDTO);
		const listTagihan = dataBulananDTO.flatMap((d) => d.pembayaran);
...
export const addDataUser = createServerFn({ method: "POST" })
	...
	.handler(async ({ data }): Promise<DataUserDTO> => {
		...
		// `nama` is the de-facto join key for monthly records, so duplicates would
		// merge two people's payment history. Rely on the unique index rather than
		// a pre-check, which would still race.
		const [row] = await db
			.insert(dataUser)
			.values({ nama })
			.onConflictDoNothing()
...
export const editDataUser = createServerFn({ method: "POST" })
	...
	.handler(async ({ data }) => {
		...
		const [clash] = await db
			.select({ id: dataUser.id })
			.from(dataUser)
			.where(eq(dataUser.nama, newName));
		if (clash) throw new Error(`Nama "${newName}" sudah terdaftar`);
...
export const addPayment = createServerFn({ method: "POST" })
	.inputValidator(
		z.object({
			...
			tanggal: z.string().regex(/^\d{6}$/, "Format bulan harus MMYYYY"),
	...
	.handler(async ({ data }) => {
		...
		// Increment in SQL rather than read-then-write. Two admins recording a
		// payment at the same time would otherwise both read the same starting
		// balance and the second write would overwrite the first, silently losing
		// one payment.
		const updated = await db
			.update(bulananUser)
			.set({ totalBayar: sql`${bulananUser.totalBayar} + ${data.nominal}` })
			.where(
				and(
		...
		if (updated.length === 0) throw new Error("Data tidak ditemukan");
